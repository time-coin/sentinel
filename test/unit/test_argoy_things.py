...
def test_valid_time_address():
    from timelib import is_valid_time_address

    main = valid_time_address()
    test = valid_time_address('testnet')

    assert is_valid_time_address(main) is True
    assert is_valid_time_address(main, 'mainnet') is True
    assert is_valid_time_address(main, 'testnet') is False

    assert is_valid_time_address(test) is False
    assert is_valid_time_address(test, 'mainnet') is False
    assert is_valid_time_address(test, 'testnet') is True


def test_invalid_time_address():
    from timelib import is_valid_time_address

    main = invalid_time_address()
    test = invalid_time_address('testnet')

    assert is_valid_time_address(main) is False
    assert is_valid_time_address(main, 'mainnet') is False
    assert is_valid_time_address(main, 'testnet') is False

    assert is_valid_time_address(test) is False
    assert is_valid_time_address(test, 'mainnet') is False
    assert is_valid_time_address(test, 'testnet') is False
...
def test_parse_masternode_status_vin():
    from timelib import parse_masternode_status_vin
...
def test_hash_function():
    ...
    hex_hash = "%x" % timelib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(timelib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(timelib.blocks_to_seconds(2)).quantize(precision) \
        == Decimal(314.4).quantize(precision)
    assert int(timelib.blocks_to_seconds(16616)) == 2612035
