...
class GovernanceObject(BaseModel):
    ...
    # sync timed gobject list with our local relational DB backend
    @classmethod
    def sync(self, timed):
        golist = timed.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_timed(timed, item)
    ...
    @classmethod
    def import_gobject_from_timed(self, timed, rec):
        import decimal
        import timelib
        import inflection
        ...
        # shim/timed conversion
        object_hex = timelib.SHIM_deserialise_from_timed(object_hex)
        objects = timelib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from timed...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from timed, with every run
        ...
        # get/create, then sync payment amounts, etc. from timed - TIMEd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(timed)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from timed! %s" % e)
            govobj.vote_delete(timed)
            return (govobj, None)
    ...
    def vote_delete(self, timed):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(timed, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, timed, signal, outcome):
        import timelib
        ...
        output = timed.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = timelib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(timed, signal)

    def sync_network_vote(self, timed, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = timed.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = ARGOD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 time addr, non-multisig
            if not timelib.is_valid_time_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid TIME address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (timelib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/TIMEDrive, etc.)
        return False
    ...
    def get_prepare_command(self):
        import timelib
        obj_data = timelib.SHIM_serialise_for_timed(self.serialise())
    ...
    def prepare(self, timed):
        try:
            object_hash = timed.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = ARGOD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not timelib.is_valid_time_address(addr, config.network):
    ...
    def hash(self):
        import timelib
        return timelib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    govobj_type = ARGOD_GOVOBJ_TYPES['watchdog']
    only_masternode_can_submit = True

    @classmethod
    def active(self, timed):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - timed.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, timed):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - timed.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, timed):
        now = int(time.time())
        return (self.created_at < (now - timed.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, timed):
        if self.is_expired(timed):
            return False

        return True

    def is_deletable(self, timed):
        if self.is_expired(timed):
            return True
