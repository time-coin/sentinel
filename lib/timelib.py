...
def is_valid_time_address(address, network='mainnet'):
    ...
    time_version = 140 if network == 'testnet' else 23
    ...
    try:
        ...
        # rescue from exception, not a valid TIME address
        return False

    if (address_version != time_version):
        return False
...
# shims 'til we can fix the timed side
def SHIM_serialise_for_timed(sentinel_hex):
    from models import ARGOD_GOVOBJ_TYPES
    # unpack
    ...
    # add 'type' attribute
    obj[1]['type'] = ARGOD_GOVOBJ_TYPES[govtype]

    # superblock => "trigger" in timed
    if govtype == 'superblock':
        obj[0] = 'trigger'

    # timed expects an array (even though there is only a 1:1 relationship between govobj->class)
    obj = [obj]

    # re-pack
    timed_hex = serialise(obj)
    return timed_hex


# shims 'til we can fix the timed side
def SHIM_deserialise_from_timed(timed_hex):
    from models import ARGOD_GOVOBJ_TYPES

    # unpack
    obj = deserialise(timed_hex)
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('time.conf')
