...
def has_time_conf():
    ...
    valid_time_conf = False

    # ensure time_conf exists & readable
    #
    # if not, print a message stating that TIME Core must be installed and
    # configured, including JSONRPC access in time.conf
    try:
        f = io.open(config.time_conf)
        valid_time_conf = True
    except IOError as e:
        print(e)

    return valid_time_conf
...
def main():
    ...
    if not has_time_conf():
        print("TIMECore must be installed and configured, including JSONRPC access in time.conf")
        sys.exit(1)
