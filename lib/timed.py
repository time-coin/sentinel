...
class TIMEDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some timed methods
    ...
    @classmethod
    def from_time_conf(self, time_dot_conf):
        from time_config import TIMEConfig
        config_text = TIMEConfig.slurp_config_file(time_dot_conf)
        creds = TIMEConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = timelib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = timelib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = timelib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_timed_proto_version_with_sentinel_ping)
